  ...
  switch (action.type) {
    case "GET_COUNT": {
      return {
        ...state,
        loading: false,
        count_value: action.value,
      };
    }
  }
};

const SiteViews = memo(
  ({ projectName, visited, getData, refresh, placeHolder, ...rest }) => {
    ...
    }, [projectName, visited, getData, refresh]);

    return (
      <span {...rest}>
        {state.loading ? placeHolder ?? "" : state.count_value}
      </span>
    );
  }
);
